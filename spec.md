...
* For the foreground form, arguments after `--` are passed first and the optional Envoy-owned prompt is appended as the final child argument. An explicitly empty prompt remains an empty argument. `list`, `status`, `wait`, and `stop` are reserved run-operation names, not agent executables.
* Foreground runs inherit stdin, stdout, and stderr. Because inherited child output cannot share a stable machine-readable stream, `--json` with a foreground run is refused before a record or process is created.
...
* **Run-event hook**: config may define `on_run_event` as a non-empty YAML argv list (executable followed by fixed arguments), invoked without a shell (fire-and-forget, non-blocking, failures logged not fatal) on every state transition with the run record path appended. This is the integration point for openfind's phone relay and anything else that wants to watch runs — Envoy publishes events; it does not build notification channels.
* Exit mapping: foreground run/`wait` return 0 on agent exit 0; nonzero agent exit → 3 with the child code preserved in `run.json`; operator Ctrl-C terminates the process group, records failed with conventional exit code 130, and returns 3; invalid combinations and ownership violations → 2; a stopped run reports stopped and `wait` returns 0 (a stop the operator requested is not a refusal).
